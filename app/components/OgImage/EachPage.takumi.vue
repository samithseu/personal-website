...
import type { CustomOGImageProps } from "@/composables/useSeo";

const { headline, title, desc } = defineProps<CustomOGImageProps>();
...
<template>
  <div class="p-16 w-full h-full bg-zinc-950">
    <div class="w-full h-full flex flex-col gap-10 relative justify-start">
      <h3
        id="headline"
        class="uppercase font-semibold font-mono text-3xl"
      ...
      <h4
        id="title"
        class="text-5xl w-3/4 text-pretty leading-11 font-semibold"
        :style="`color: white;`"
      >
        {{ title }}
      </h4>
      <p
        id="desc"
        class="text-3xl w-2/3 text-pretty leading-relaxed"
        :style="`color: oklch(70.5% 0.015 286.067);`"
      >
        {{ desc }}
      </p>
